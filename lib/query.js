...
/**
 * Filter the set of documents found before to return the result with the given `sfield` and `value`.
 *
 ...
 * @example
 * var query = client.createQuery();
 * query.q({ '*' : '*' }).geoFilter('bbox', 'sfield', (37.774,-122.419), 20);
 * fq={!bbox%20sfield=cloc%20pt=37.774,-122.419%20d=20}
 */

Query.prototype.geoFilter= function(func, field, lat, lon, radius){
   var self = this;
   var q = '';

   q += "!" + func + " sfield=" + field + " pt=" + lat + "," + lon + " d=" + radius;
   console.log("Subquery = " + q);
   var parameter = 'fq={' + q.replace(/\s+/g, '%20') + '}';
